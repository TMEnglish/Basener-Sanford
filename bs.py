...
sns.set() # Use the Seaborn package to generate plots

# Default number of frames in an animation (None yields a static figure).
...
# TO DO: Redefine these in terms of delta
N_RATES = {
            'Gamma'     : 500 + 1,            # Sect 5.4
            'Gaussian'  : 5 * 200 // 4 + 1,   # Sect 5.3
            'NoneExact' : 5 * 500 // 4 + 1,    # Sects 5.1, 5.2
            'None'      : 5 * 500 // 4 + 1    # Sects 5.1, 5.2
          }
...
class WrappedTrajectory(Evolution):
    def __init__(self, trajectory, growth_rates, label=''):
        ...
        self.annual_growth_rates = np.array(growth_rates)
        self.label = label
...
class CompareProcesses(object):
    ...
    def animate(self, nframes, line_styles=None):
        """
        Return animation of one or more evolutionary processes.
        
        If the stride is None, then a static figure is returned instead of
        an animation.
        """
        if nframes is None:
            stride = None
        ...
        for i in range(len(x)):
            lines[i], = ax.plot(x[i], p[i][-1], label=labels[i],
                                ls=line_styles[i], zorder=10)
        for i in range(len(x)):
            ax.plot(x[i], p[i][0], c='black', lw=1, alpha=0.5, zorder=3)
            ax.plot(x[i], p[i][-1], c=lines[i].get_c(), lw=1)
        ax.set_title('Evolution for {0} Years{1}'.format(n-1, self.subtitle))
        ...
        if stride is None:
            ...
            graphic = animation.FuncAnimation(fig, animator, init_func=initializer,
...
def old_mean_and_variance(x, p):
    ...
    def expectation(w):
        if w.ndim == 2:
            return np.sum(np.multiply(p, w), axis=1)
        else:
            return np.dot(p, w)

    mean = expectation(x)
    variance = expectation(np.square(x))
    variance -= np.square(mean)
    return mean, variance
